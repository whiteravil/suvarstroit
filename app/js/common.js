$(function() {
	...
	function locationScroll() {
		...
		cityMap.each(function() {
			...
			locNav.find('> div').each(function() {
				...
				// let thsLink = $(this);
				// console.log(scrTop, thsCityMap.offset().top - thsLink.outerHeight())
				// if ( scrTop > thsCityMap.offset().top - thsLink.outerHeight() ) {
				// 	thsLink.addClass('white')
				// }
				// else {
				// 	thsLink.removeClass('white')
  	...
  	if ( $(window).width() >= 992 ) {
	  	...
	  	if ( headerContentWidth > $(window).width() ) {
	  		...
	  		for ( let i = $('.main-footer-nav > ul > li').length - 1; i--; i >= 0 ) {
	  			...
	  			if ( headerContentWidth > $(window).width() && !ths.hasClass('has-child') ) {
  ...
  smallStocksSlider.slick({
	...
	liveInRescom.on('beforeChange', function(e, s, c, n) {
		liveInRescom.find('.slick-slide').removeClass('prev-slide').eq(n - 1).addClass('prev-slide')
	});
	...
	let videosSlider = $('.videos-slider').slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		arrows: false,
		dots: false,
